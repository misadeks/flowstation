//! SN-PAGE RESPONSE (MS → SwMI), SN-PDU type 10, subtype 1.
...
const PAGE_SUBTYPE_RESPONSE: u64 = 1;
...
impl PageResponse {
    pub fn from_bitbuf(buffer: &mut BitBuffer) -> Result<Self, PduParseErr> {
        ...
        if subtype != PAGE_SUBTYPE_RESPONSE {
        ...
        Ok(PageResponse { nsapi })
    }

    pub fn to_bitbuf(&self, buffer: &mut BitBuffer) -> Result<(), PduParseErr> {
        buffer.write_bits(SnPduType::Page.into_raw(), 4);
        buffer.write_bits(PAGE_SUBTYPE_RESPONSE, 1);
...
mod tests {
    ...
    fn round_trip_with_optionals() {
        let pdu = PageResponse { nsapi: Nsapi(12) };
        let mut buf = BitBuffer::new_autoexpand(32);
        pdu.to_bitbuf(&mut buf).unwrap();
        let bits = buf.to_bitstr();
        buf.seek(0);
